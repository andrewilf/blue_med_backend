...
//Getting all past apoointments and populate
router.get("/populated/all", async (req, res) => {
  const pastAppointmentAll = await pastAppointment
    .find({})
    .populate("doctor")
    .populate("patient");
  console.log(pastAppointmentAll);
...
router.get("/populated/:appID", async (req, res) => {
  try {
    const appID = req.params.appID;
    console.log("searched for schedule by ID");
    const getApp = await pastAppointment
      .findOne({ _id: appID })
      .populate("doctor")
      .populate("patient");
    if (getApp != null) {
...
router.delete("/:pastAppID", async (req, res) => {
  try {
    ...
    if (pastAppDel.deletedCount !== 0) {
      ...
      res.status(404).send("No past App was found with that ID");
    }
  } catch (error) {
    console.error(error);
